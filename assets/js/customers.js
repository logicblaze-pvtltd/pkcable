...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function customerRoleLabel(role) {
        ...
        return value === '' ? 'Customer' : value.replace(/\b\w/g, function (character) {
    ...
    function customerStatusLabel(status) {
        ...
        return value === '' ? 'Active' : value.replace(/\b\w/g, function (character) {
            return character.toUpperCase();
        });
    }
    ...
    function normalizeCustomerData(data) {
        const source = data || {};
        const customerId = extractNumericCustomerId(source.id ?? source.id_display ?? source.customer_id ?? null);
        const packageId = extractNumericPackageId(source.package_id ?? source.packageId ?? source.package ?? null);
        const createdAtRaw = String(source.created_at ?? source.createdAt ?? '').trim();
        const createdAtDisplay = String(source.created_at_display ?? source.createdAtDisplay ?? '').trim() || formatDateDisplay(createdAtRaw);

        return {
            id: customerId,
            idDisplay: String(source.id_display ?? '').trim() || (customerId !== null ? formatCustomerId(customerId) : ''),
            name: String(source.name ?? '').trim(),
            email: String(source.email ?? '').trim(),
            password: String(source.password ?? '').trim(),
            userRole: String(source.user_role ?? source.userRole ?? 'customer').trim().toLowerCase() || 'customer',
            status: String(source.status ?? 'active').trim().toLowerCase() || 'active',
            packageId,
            packageName: String(source.package_name ?? source.packageName ?? '').trim(),
    ...
    function parseCustomerRow(row) {
        ...
        const nameElement = firstCell?.querySelector('[data-role="name"]') || firstCell?.querySelector('p:first-child');
        const idElement = firstCell?.querySelector('[data-role="id"]') || firstCell?.querySelector('p:nth-child(2)');
        const emailElement = rowElement?.querySelector('[data-role="email"]') || rowElement?.querySelector('td:nth-child(2)');
        const packageNameElement = rowElement?.querySelector('[data-role="package-name"]') || rowElement?.querySelector('td:nth-child(3)');
        const roleElement = rowElement?.querySelector('[data-role="role-badge"]') || rowElement?.querySelector('td:nth-child(4)');
        const statusElement = rowElement?.querySelector('[data-role="status-badge"]') || rowElement?.querySelector('td:nth-child(5)');
        const createdElement = rowElement?.querySelector('[data-role="created"]') || rowElement?.querySelector('td:nth-child(6)');
        const mobileEmail = firstCell?.querySelector('[data-role="mobile-email"]');
        const mobilePackageName = firstCell?.querySelector('[data-role="mobile-package-name"]');
        const mobilePackageId = firstCell?.querySelector('[data-role="mobile-package-id"]');
        const mobileRoleBadge = firstCell?.querySelector('[data-role="mobile-role-badge"]');
        const mobileAddress = firstCell?.querySelector('[data-role="mobile-address"]');
        const mobileCreated = firstCell?.querySelector('[data-role="mobile-created"]');

        const idValue = dataset.id || idElement?.textContent?.replace(/^ID:\s*/i, '') || '';
        const packageIdValue = dataset.packageId || mobilePackageId?.textContent || packageNameElement?.textContent || '';

        return {
            row: rowElement,
            id: extractNumericCustomerId(idValue),
            idDisplay: dataset.idDisplay || idElement?.textContent?.replace(/^ID:\s*/i, '') || '',
            name: dataset.name || nameElement?.textContent?.trim() || '',
            email: dataset.email || emailElement?.textContent?.trim() || mobileEmail?.textContent?.trim() || '',
            userRole: String(dataset.userRole || mobileRoleBadge?.textContent || roleElement?.textContent || 'customer').trim().toLowerCase(),
            status: String(dataset.status || statusElement?.textContent || 'active').trim().toLowerCase(),
            packageId: extractNumericPackageId(dataset.packageId || packageIdValue || ''),
            packageName: dataset.packageName || mobilePackageName?.textContent?.trim() || packageNameElement?.textContent?.trim() || '',
            address: dataset.address || mobileAddress?.textContent?.trim() || '',
            createdAtDisplay: dataset.createdAt || createdElement?.textContent?.trim() || mobileCreated?.textContent?.trim() || '',
        };
    }
    ...
    function getCustomerRowElement(row) {
        ...
        const rowId = extractNumericCustomerId(typeof row === 'object' ? row.id : row);
        if (rowId === null) {
            return null;
        }

        return customersTable?.querySelector(`tr.user-row[data-id="${rowId}"]`) || null;
    }

    function getNextCustomerId() {
        const rows = customersTable ? customersTable.querySelectorAll('tbody tr.user-row[data-id]') : [];
        let highestId = 0;
    ...
    function getPackageDisplayName(customer) {
        ...
        return customer.packageId !== null ? formatPackageId(customer.packageId) : 'No Package';
    }

    function getPackageDisplayId(customer) {
        return customer.packageId !== null ? formatPackageId(customer.packageId) : 'No package assigned';
    }
    ...
    function setRowDataAttributes(rowElement, customer) {
        ...
        rowElement.dataset.packageId = customer.packageId !== null ? String(customer.packageId) : '';
        rowElement.dataset.packageName = customer.packageName || getPackageDisplayName(customer);
    ...
    function buildCustomerRowMarkup(data) {
        ...
        const packageIdAttribute = customer.packageId !== null ? escapeHtml(customer.packageId) : '';
    ...
    function addCustomerToTable(data) {
        ...
        refreshTable();
        const newId = extractNumericCustomerId(data?.id ?? data?.id_display ?? null);
    ...
    function replaceCustomerRow(row, data) {
        ...
        const updatedId = customer.id !== null ? customer.id : extractNumericCustomerId(data?.id ?? data?.id_display ?? null);
    ...
    function ensureSubscriptionCustomerOption(customer) {
        if (!subUserId || !customer || customer.id === null || customer.id === undefined) return;

        const customerId = String(customer.id);
        const customerLabel = `${String(customer.name || '').toUpperCase()} (#U${String(customer.id).padStart(3, '0')})`;
        let option = Array.from(subUserId.options || []).find((item) => String(item.value) === customerId) || null;
    ...
    function openCustomerSubscriptionModal(customer, packageId = null, rowElement = null) {
        ...
        if (subPackageId) {
            subPackageId.value = packageId ? String(packageId) : (customer?.packageId !== null && customer?.packageId !== undefined ? String(customer.packageId) : '');
    ...
    function sendPendingWelcomeEmailFallback() {
        if (!pendingWelcomeEmail || welcomeEmailSent || !pendingWelcomeEmail.password) {
        ...
        return sendCustomerRequest(`${customerApiBase}/send-welcome-email.php`, {
            ...
            if (!result?.status) {
                console.error('Welcome email fallback failed:', result?.message || result?.data?.email_error);
            }
    ...
    function closeCustomerSubscriptionModal() {
        ...
        const shouldSendFallback = Boolean(pendingWelcomeEmail && !welcomeEmailSent);
    ...
    function syncCustomerRowAfterSubscription(subscriptionData) {
        ...
        const merged = {
            ...
            package_name: subscriptionData?.package_name ?? getPackageLabelFromSelect(),
            status: 'active',
        };

        if (pendingSubscriptionRow) {
            pendingSubscriptionRow = replaceCustomerRow(pendingSubscriptionRow, merged);
        } else {
            replaceCustomerRow(pendingSubscriptionCustomer.id, merged);
        }
    }

    if (subscriptionModal && subStartDateDisplay && subStartDateHidden && subStartCalModal && subStartCalPanel && subStartCalMonth && subStartCalGrid && subStartCalPrev && subStartCalNext && subStartCalCancel && subStartCalToday && subEndDateDisplay && subEndDateHidden && subEndCalModal && subEndCalPanel && subEndCalMonth && subEndCalGrid && subEndCalPrev && subEndCalNext && subEndCalCancel && subEndCalToday) {
    ...
    function setModalState(mode, row = null) {
        ...
        if (mode === 'edit' && activeRow) {
            ...
            userPackageSelect.value = data.packageId !== null ? String(data.packageId) : '';
    ...
    function openViewModal(row) {
        ...
        setBadge(viewRole, roleLabel, customerRoleClasses(data.userRole), 'inline-flex px-3 py-1 rounded-full text-xs font-medium');
        setText(viewPackage, packageName);
        setText(viewPackageId, packageId);
        setBadge(viewStatus, statusLabel, customerStatusClasses(data.status), 'inline-flex px-3 py-1 rounded-full text-xs font-medium');
    ...
    function getFormData() {
        return {
            id: activeRow ? extractNumericCustomerId(activeRow.getAttribute('data-id')) : null,
            ...
            return sendCustomerRequest(`${customerApiBase}/create.php`, data)
                .then(function (result) {
                    if (result?.status) {
                        ...
                        // Extract the nested user object for the table row
                        ...
                        const openSubscriptionFlow = function () {
                            if (subscriptionModal) {
                                openCustomerSubscriptionModal(addedUser, data.package, createdRow);
                            }
                        };

                        const welcomePassword = result.data?.welcome_password || result.data?.generated_password || data.password || '';
                        pendingWelcomeEmail = {
                            userId: addedUser.id,
                            name: addedUser.name,
                        ...
                        const emailNote = 'Login and package details will be emailed after the subscription is saved.';
